...
class JetParticleDataset(torch.utils.data.Dataset):
    def __init__(self, df, device='cpu', transform=None):
        ...
        self.jet_features = df[['iEvent', 'jet_pt', 'jet_eta', 'jet_phi', 'jet_E', 'jet_count']]
        self.particle_features = df[['part_PID', 'part_Charge', 'track_d0', 'track_dz', 'track_d0_sig', 'track_dz_sig']]  #  'part_pt'
...
class PairEmbed(nn.Module):
    def __init__(self, num_heads, in_channels, out_channels):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=5, stride=5)
    ...
    def forward(self, four_momenta):
        ...
        input_tensor = input_tensor.view(N*self.num_heads, 10, 10)

        return input_tensor


class ParticleBlock(nn.Module):
    def __init__(self,embed_dim, num_heads=4,ffn_ratio=4,
...
class Part(nn.Module):
    def __init__(self, input_dim, embed_dim=32, num_heads=4, num_layers=4, num_cls_layers=2):
        ...
        self.embed = Embed(input_dim, embed_dim)      # this can be made multi layered as the original work
